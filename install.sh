...
print_success() {
    ...
    printf "\n"
    printf "  ${DIM}# Mount the filesystem${RESET}\n"
    printf "  sudo obsfs mount /obs\n"
    printf "\n"
    printf "  ${DIM}# Read some metrics${RESET}\n"
    printf "  cat /obs/system/cpu/usage\n"
    printf "  cat /obs/system/memory/available\n"
    printf "  cat /obs/health\n"
    printf "\n"
    printf "  ${DIM}# Watch in real-time${RESET}\n"
    printf "  while true; do clear; cat /obs/health; sleep 1; done\n"
    printf "\n"
    printf "  ${DIM}# Unmount when done${RESET}\n"
    printf "  sudo obsfs unmount /obs\n"
    printf "\n"
